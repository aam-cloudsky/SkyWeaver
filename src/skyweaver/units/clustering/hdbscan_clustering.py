...
from skyweaver.units.clustering.base_clustering import BaseClustering
from skyweaver.units.clustering.cluster import Cluster
from skyweaver.units.clustering.cluster_shape_builder import ClusterShapeBuilder
...
class HDBSCANClustering(BaseClustering):
    ...
    def _fit_few_points(
        self, points: List[Point], zone_type: ZoneType
    ) -> List[Cluster]:
        ...
        print(
            f"[INFO] Too few {zone_type} points ({len(points)}). Creating one cluster."
        )
    ...
    def _build_cluster(self, source_points: list[Point], zone_type: ZoneType):
        ...
        polygon = self.shape_builder.smooth_radial_boundary(
            centroid=centroid, source_points=source_points, offset=80.0
        )
        return Cluster(
    ...
    def _fit_points(self, points: List[Point], zone_type: ZoneType) -> List[Cluster]:
        ...
        for label in unique_labels:
            if label == -1:
                continue
            source_points = [
                points[i] for i in range(len(points)) if labels[i] == label
            ]
    ...
    def fit_points(self, points: List[Point]) -> List[int]:
        labels, _ = self._labeling(points)
        return labels.tolist()

    def fit_zone(self, points: List[Point], zone_type: ZoneType) -> List[Cluster]:
        return self._fit_points(points, zone_type)
